...
"""
    ...
    This node make the Sawyer's face follow yours. It seems to keep looking at you.

    Before running this node. You have to open the camera on the head of Sawyer.
...
import cv2
import roslib
#import roslib.load_manifest('demos_sysu_baxter')
import rospy
...
class FaceFollower():
    ...
    def get_camera_frame(self,img_data):
        bridge = CvBridge()
        try:
            cv_image=bridge.imgmsg_to_cv2(img_data,"bgr8")
        except CvBridgeError:
            return
        self.r_face(cv_image)
    ...
    def r_face(self,msg):
        ...
        for (x,y,h,w) in faces:
            msg=cv2.rectangle(msg,(x,y),(x+w,y+h),(255,0,0),2)
            print("face position:x=%d,y=%d,h=%d,w=%d" %(x,y,h,w))
        cv2.imshow("faces",msg)
